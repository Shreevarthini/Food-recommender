...
def main():
    ...
    try:
        ...
        print(f"Yes -  Loaded {len(food_items)} food items successfully")
        ...
        print(f"No -  Error initializing system: {error}")
...
def interactive_food_chatbot(collection):
    ...
    while True:
        try:
            ...
            print(f"YNo-  Error processing request: {e}")
...
def handle_food_search(collection, query):
    ...
    if not results:
        print("No -  No matching foods found.")
    ...
    print(f"\nYes -  Found {len(results)} recommendations:")
    print("=" * 60)
